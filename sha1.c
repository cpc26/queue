/*Originally part of Gnu privacy guard.*/
...
char *sha1(char *strin)
{
static char strout[129];
register int i;
byte *out;
SHA1_CONTEXT context;
sha1_init (&context);
sha1_write(&context,(byte*) strin,(size_t) strlen(strin));
sha1_final(&context);
out = sha1_read(&context);
/*Convert to hexidecimal string.*/
for(i=0;i<64;++i) sprintf(strout+i*2,"%02x",(unsigned char) *(out+i));
/*fprintf(stderr,"%s\n", strout);*/
  return(strout);
}
...
static
#endif
const char * const gnupgext_version = "SHA1 ($Revision: 1.2 $)";
